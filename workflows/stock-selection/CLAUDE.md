...
**Phase 8 (2026-05-31, EXPLORATORY) — quality probe FAILS too.** A pre-registered mcap-free quality composite (ROE + GP/A + magic-formula rank; immune to the split bug by construction) was built to test whether quality is the best free-data directional bet: **quality_composite_v1 ExSharpe = −0.057, raw_p 0.596, 90% CI [−0.441,+0.349], strat Sharpe 0.795 vs SPY 0.867 — FAIL** (complete 202-date panel). It CONTRADICTS the pre-registered +0.15..+0.30 — **the ROE edge (+0.242 standalone) does NOT survive naive equal-weight quality combination**, coming in mildly negative. The strongest corrected survivor is fragile, not deployable. **Net: quality, like value, yields no free-data edge beating SPY; gate stays 0/N across the entire program.** The mcap split-adjust bug is now FIXED in the shared engine (raw-price mcap, `pytest tests/stock/` 119 passed). See `research/log.md` (Phase 8) + `next_steps_2026-05-30.md`.
