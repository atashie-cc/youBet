...
- `workflows/factor-timing/` — Ken French factor portfolio timing COMPLETE (15 phases, 3 Codex rounds, 8 bugs fixed). 62yr daily data, 5 global regions. **8/18 PASS** on paper. Hedged VLUE passes (ExSh +0.795, p=0.030). Independent timing (Sharpe 1.709) beats rotation. **CMA passes Holm/6 in ALL 3 intl regions (most robust factor).** Transfer: 19/24 positive, 6/24 significant. Regime gate: factor breadth identifies favorable periods (alpha 2-3x higher, p=0.005) but always-on timing still works. Net ExSharpe +0.453 weekly after costs.
