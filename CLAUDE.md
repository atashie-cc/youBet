...
- `workflows/factor-timing/` — Ken French factor portfolio timing COMPLETE (10 phases, 3 Codex rounds, 8 bugs fixed). 62yr daily data, 5 global regions. **Phase A: 8/18 PASS** on paper factors. **Phase B: hedged VLUE passes (ExSh +0.795, p=0.030)**. **Phase C: weekly is optimal frequency.** **Phase D: replicates in 4/5 regions** (HML, SMB positive globally). Timing alpha is bear-driven, distributed across VIX regimes/eras. Asia-Pac fails (short drawdowns → whipsaw). Multi-region diversification additive (corr 0.04-0.18). **After implementation costs: net ExSharpe +0.372 weekly (base case), break-even at ~4% annual costs.** Under pessimistic costs, barely positive. VTI SMA drawdown overlay remains most robust practical finding.
